...
internal sealed class Settings : IUiComponent
{
    ...
    internal Settings(PluginUi ui)
    {
        ...
        this.Tabs = new List<ISettingsTab> {
            ...
            new Database(this.Mutable, this.Ui.Plugin),
        ...
        this.Ui.Plugin.Commands.Register("/chat3", "Perform various actions with Chat 3.").Execute += this.Command;
    ...
    {
        this.Ui.Plugin.Interface.UiBuilder.OpenConfigUi -= this.Toggle;
        this.Ui.Plugin.Commands.Register("/chat3").Execute -= this.Command;
    }
    ...
    public void Draw()
    {
        ...
        if (!ImGui.Begin($"{name}###chat3-settings", ref this.Ui.SettingsVisible, ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse))
        {
            ImGui.End();
            return;
        }
        ...
        if (ImGui.BeginTable("##chat3-settings-table", 2))
        {
            ...
            if (ImGui.BeginChild("##chat3-settings", new Vector2(-1, height)))
            {
