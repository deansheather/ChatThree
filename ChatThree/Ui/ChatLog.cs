...
internal sealed class ChatLog : IUiComponent
{
    ...
    private const string AutoCompleteId = "##chat3-autocomplete";
    ...
    internal ChatLog(PluginUi ui)
    {
        ...
        this.Ui.Plugin.Commands.Register("/clearlog2", "Clear the Chat 3 chat log").Execute += this.ClearLog;
        this.Ui.Plugin.Commands.Register("/chat3").Execute += this.ToggleChat;
    ...
    private unsafe bool DrawChatLog()
    {
        ...
        if (!ImGui.Begin($"{Plugin.Name}###chat3", flags))
        {
        ...
        ImGui.InputText("##chat3-input", ref this.Chat, 500, inputFlags, this.Callback);
    ...
    {
        if (ImGui.BeginChild("##chat3-messages", new Vector2(-1, childHeight)))
        {
            ImGui.PushStyleVar(ImGuiStyleVar.ItemSpacing, Vector2.Zero);
    ...
    private int DrawTabBar()
    {
        ...
        if (!ImGui.BeginTabBar("##chat3-tabs"))
        {
            return currentTab;
        }
    ...
    private int DrawTabSidebar()
    {
        ...
        if (ImGui.BeginChild("##chat3-tab-sidebar", new Vector2(-1, childHeight)))
        {
            for (var tabI = 0; tabI < this.Ui.Plugin.Config.Tabs.Count; tabI++)
            {
                var tab = this.Ui.Plugin.Config.Tabs[tabI];
